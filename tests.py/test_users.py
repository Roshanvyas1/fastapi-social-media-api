...
async def test_user_wrong_email_or_password_format_return_422(client, email, password, status_code):
    response = await client.post("/users/", json={
        "email": email,
        "password": password
    })
...
async def test_user_insufficient_password_length_less_than_8_charactor_return_422(client):
    response = await client.post("/users/", json={
        "email": "dummy@gmail.com",
        "password": "dummy"
    })
