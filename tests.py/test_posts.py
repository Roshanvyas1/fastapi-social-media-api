...
async def test_create_post_return_201(authorized_client, test_user):
    response = await authorized_client.post("/posts/", json={
        "title": "Dummy Title",
        "content": "Dummy Content",
        "published": False
    })
...
@pytest.mark.parametrize("title, content, published, status_code", [
    ("", "dummy content", True, 422),
    ("dummy title", "", False, 422)
])
async def test_create_post_title_or_content_cannot_be_empty_return_422(authorized_client, title, content, published, status_code):
    response = await authorized_client.post("/posts/", json={
        "title": title,
        "content": content,
        "published": published
...
async def test_create_post_title_length_cannot_be_more_than_120_words_return_422(authorized_client):
    response = await authorized_client.post("/posts/", json={
        "title": "a" * 121,
        "content": "dummy content",
        "published": False
...
# Testing delete post.
async def test_unauthorized_user_delete_post_using_post_id_return_401(client, test_post):
    id = test_post.id
    response = await client.delete(f"/posts/{id}")
    assert response.status_code == 401

async def test_delete_post_using_post_id_belongs_to_another_user_return_403(authorized_client, test_post2):
    id = test_post2.id
    response = await authorized_client.delete(f"/posts/{id}")
    assert response.status_code == 403

async def test_delete_post_using_post_id_belongs_to_owner_return_204(authorized_client, test_post):
    id = test_post.id
    response = await authorized_client.delete(f"/posts/{id}")
    assert response.status_code == 204

async def test_delete_post_using_post_id_that_do_not_exist_return_404(authorized_client):
    id = 99999
    response = await authorized_client.delete(f"/posts/{id}")
    assert response.status_code == 404
...
async def test_unathorized_user_update_post_using_post_id_return_401(client, test_post):
    id = test_post.id
    response = await client.patch(f"/posts/{id}", json={
        "title": "Update Dummy Title",
        "published": False
    })
    assert response.status_code == 401

async def test_update_post_using_post_id_belongs_another_user_return_403(authorized_client, test_post2):
    id = test_post2.id
    response = await authorized_client.patch(f"/posts/{id}", json={
        "title": "Update Dummy Title",
        "published": False
    })
    assert response.status_code == 403
