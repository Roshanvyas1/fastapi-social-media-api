...
async def test_vote_post_id_return_200(authorized_client, test_post):
    response = await authorized_client.post(
        "/votes/", json={"post_id": test_post.id, "dir": 1}
    )
...
async def test_vote_post_id_already_exist_return_409(authorized_client, test_vote):
    response = await authorized_client.post(
        "/votes/", json={"post_id": test_vote.post_id, "dir": 1}
    )
...
async def test_unvote_post_id_return_200(authorized_client, test_vote):
    response = await authorized_client.post(
        "/votes/", json={"post_id": test_vote.post_id, "dir": 0}
    )
    assert response.status_code == 200
...
async def test_unvote_post_id_in_which_vote_does_not_exist_return_404(
    ...
    response = await authorized_client.post(
        "/votes/", json={"post_id": test_post.id, "dir": 0}
    )
    assert response.status_code == 404
